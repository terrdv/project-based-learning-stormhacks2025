...
export async function generateAppGuide({ userInfo, projectDetails }, userId) {
  const response = await ai.models.generateContent({
    ...
    B. A list of files that will be needed for the project, filled with boilerplate code, comments, and placeholders where the user will need to add their own code. The boilerplate doesn't necessarily need to be completely barebones but should not implement core functionality for the user. Include only minimal code for the language the user wants to learn, just enough to illustrate structure. For other languages in the project, you may provide normal scaffolding. Use comments or placeholders to indicate where the user should write code. For the file contents, add line breaks where appropriate to make it more readable.
